 ...
 * from {@code vhdl/simulations/ym2149_audio/commit_46ee39d/tb_feat_ym2149.vcd.zip}.
 *
 * <p>The testbench ({@code vhdl/testbench/tb_feat_ym2149.vhd}) exercises all
 * YM-2149 features across 10 ms: tone generation, noise, mixer, and all eight
 * envelope shapes, plus single-shot hold variants and register read-back.
 *
...
class FeatYm2149ConformanceTest {
    ...
    // -----------------------------------------------------------------------
    // VCD identifiers (top-level testbench scope in tb_feat_ym2149.vcd)
    // -----------------------------------------------------------------------
    ...
    private static final String ID_CH_A    = ")";   // ch_a_o
    ...
    /** Clock period in femtoseconds (~21.477 MHz master clock: 46.560852 ns). */
    ...
    /**
     * Drive the Java {@link Ym2149Audio} model with the same stimuli as
     * {@code tb_feat_ym2149.vhd} and assert that all output signals match the
     * VCD at every timestamp where the VHDL simulation records a change.
     *
     * <p>At least 250 distinct output-change timestamps must be verified to
     * ensure the test exercises meaningful behavior.
     *
     * <p><b>PSG clock-enable timing note:</b> The testbench generates
     * {@code en_clk_psg_i} from a registered counter; the Java model asserts
     * {@code enClkPsgI} when {@code clkPsgR == 2}, mirroring the same
     * convention used in {@link Ym2149AudioTest}.  Because Java's computed
     * output values persist until the next enable, the comparison against the
     * VCD (which records changes one master clock later) still holds.
     */
    @Test
    ...
    private Path findVcdZip() {
        ...
        while (base != null) {
            Path candidate = base.resolve(
                "vhdl/simulations/ym2149_audio/commit_46ee39d/tb_feat_ym2149.vcd.zip");
            if (Files.exists(candidate)) return candidate;
            base = base.getParent();
        }
        throw new RuntimeException("Cannot find tb_feat_ym2149.vcd.zip");
    }

    private File extractVcd(Path zipPath) throws IOException {
        File tmp = File.createTempFile("tb_feat_ym2149", ".vcd");
        tmp.deleteOnExit();
